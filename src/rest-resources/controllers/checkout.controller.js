...
export class CheckoutController {
  static async createCheckout(req, res, next) {
    try {
      const result = await new CreateCheckoutService(req.context).create({ ...req.body, userId: req.user.id, key: req.headers['idempotency-key'] || req.headers['Idempotency-Key'] || req.headers['x-idempotency-key'] })
  ...
  static async validateCheckout(req, res, next) {
    try {
      const result = await new ValidateCheckoutService(req.context).validate({ ...req.body, userId: req.user.id })
  ...
  static async applyCoupon(req, res, next) {
    try {
      const result = await new ApplyCouponService(req.context).apply({ ...req.body, userId: req.user.id })
  ...
  static async selectAddress(req, res, next) {
    try {
      const result = await new SelectAddressService(req.context).select({ ...req.body, userId: req.user.id })
  ...
  static async delivery(req, res, next) {
    try {
      const result = await new DeliveryService(req.context).choose({ ...req.body, userId: req.user.id })
